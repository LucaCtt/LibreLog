...
def get_logs_from_group(group_list):
	...
	for ele in group_list:
		logs_from_group.append(ele["text"])
...
class LogParser:
	...
	def __store_regex_for_logs(self, result_list, group_dict_list, log_regex):
		...
		for log in wrong_logs:
			result_list.append((log["text"], self.new_event, log_regex))
			self.new_event = self.new_event + 1
		return result_list
	...
	def __generate_prompt_with_log_list(self, log_list, is_dict=False):
		"""
		...
		trimmed_log_list = self.__adaptive_random_sampling(
			log_list, self.regex_sample, is_dict=is_dict
		)
		messages = [
			{
				"role": "system",
	...
	def __jaccard_distance(self, text1, text2):
		"""
	...
	def __min_distance(self, candidate_set, target_set):
		"""
		...
		for candidate in candidate_set:
			...
			for target in target_set:
				if self.similarity_measure == "cosine":
					...
					min_distance = min(
						min_distance, self.__jaccard_distance(candidate, target)
					)
				else:
	...
	def __adaptive_random_sampling(
		self, logs, sample_size, max_logs=200, similarity_flag=False, is_dict=False
	):
		"""
		...
		if self.similarity_measure == "random":
			...
			for _ in range(sample_size):
				if not sample_list:
					...
					candidate_distances = self.__min_distance(logs, selected_logs)
					if similarity_flag:
	...
	def __check_regex_from_groups(
		...
		for log in group_dict_list:
			if self.do_self_reflection == "True":
				if verify_one_regex(log["text"], log_regex):
					self.regex_manager.add_regex_template(log_regex, log["text"])
					if new_event == 0:
						result_list.append((log["text"], log["template_id"], log_regex))
					else:
						result_list.append((log["text"], new_event, log_regex))
				else:
	...
	def __remove_first_matching_item(self, data, content_to_remove):
		...
		for i, item in enumerate(data):
			if item["text"] == content_to_remove:
				del data[i]
				break
		return data
